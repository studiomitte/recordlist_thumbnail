...
namespace StudioMitte\RecordlistThumbnail\Xclass\V13;
...
class XclassedDatabaseRecordList extends DatabaseRecordList {
    ...
    public function renderListRow($table, array $row, int $indent, array $translations, bool $translationEnabled)
    {
        ...
        $isTypo312Up = (new Typo3Version())->getMajorVersion() >= 12;
        // xclass end
    ...
    public function __renderListRow($table, array $row, int $indent, array $translations, bool $translationEnabled)
    {
        // xclass begin
        $thumbnailField = $this->getThumbnailField($table);
        $isTypo312Up = (new Typo3Version())->getMajorVersion() >= 12;
        ...
        foreach ($this->fieldArray as $fCol) {
            if ($fCol === $titleCol) {
                ...
                $theData[$fCol] = $row[$fCol];
            } elseif ($fCol !== '' && $fCol === ($GLOBALS['TCA'][$table]['ctrl']['cruser_id'] ?? '') && !$isTypo312Up) {
